...
def verify_inputs(input_list, index=0):
    if index == len(input_list):
        return
    
    try:
        if type(input_list[index][0]) != input_list[index][1]:
            return f'An error occured! {input_list[index][0]} is not of type {input_list[index][1]} but of type {type(input_list[index][0])}'
            
        else:  verify_inputs(input_list, index+1)
    except:  return 'An error occured! You either provided an invalid type or an invalid input'


def make_updates(fields, values, obj):
    for ind, field in enumerate(fields):
        try:
            obj.__setattr__(fields[ind], values[ind])
        except:  return 'An error occurred! Please check that correct data is passed in'
